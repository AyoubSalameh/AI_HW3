...
tree = """root: split by 'Smell_Loss' if the value >= 1.0
root->true: is a Leaf with predictions {'T': 3}
root->false: split by 'Fever' if the value >= 1.0
root->false->true: split by 'Cough' if the value >= 0.5
root->false->true->true: is a Leaf with predictions {'T': 1}
root->false->true->false: is a Leaf with predictions {'F': 1}
root->false->false: is a Leaf with predictions {'F': 2}"""

tree_pruning = """root: split by 'Smell_Loss' if the value >= 1.0
root->true: is a Leaf with predictions {'T': 3}
root->false: split by 'Fever' if the value >= 1.0
root->false->true: is a Leaf with predictions {'F': 1, 'T': 1}
root->false->false: is a Leaf with predictions {'F': 2}"""
...
def test_initiation(attributes_names):
    print("\n########## test_initiation ##########")
    id3 = ID3(attributes_names, target_attribute="Corona")
...
def test_find_best_split(attributes_names, x_train, y_train):
    ...
    assert best_question.column == 'Smell_Loss', f"Best question is not as expected: {best_question.column}"
...
def test_predict(attributes_names, x_train, y_train, x_test, y_test):
    ...
    assert accuracy_val == 1, f"Accuracy is not as expected: {accuracy_val}"
...
def test_min_for_pruning(attributes_names, x_train, y_train):
    ...
    id3 = ID3(attributes_names,3, target_attribute="Corona")
    id3.fit(x_train, y_train)
...
def load_data_set_t(clf_type: str):
    ...
    train_file_path = dataset_path + "\\Corona_Symptoms_Data.csv"
    test_file_path = dataset_path + "\\Corona_Symptoms_Test_Data.csv"
